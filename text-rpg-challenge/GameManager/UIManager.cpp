...
namespace TextRPG
{
	namespace Manager
	{
		...
		{
			// 1. 출력용 문자열 생성
			std::string hp_display = std::to_string(player.GetHP()) + "/" + std::to_string(player.GetMaxHP());
			std::string mp_display = std::to_string(player.GetMP()) + "/" + std::to_string(player.GetMaxMP());
			std::string atk_display = std::to_string(player.GetAttack());
			std::string def_display = std::to_string(player.GetDefense());
